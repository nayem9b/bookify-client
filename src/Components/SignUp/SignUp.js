          ...
          <div className='hidden lg:flex bg-gradient-to-br from-indigo-600 to-purple-600 p-12 flex-col justify-center relative overflow-hidden'>
            ...
            >
              ...
              <p className='text-indigo-100 text-lg mb-8'>
                Create your account and discover a world of books. Join thousands of readers and sellers today!
              </p>
              ...
              <div className='grid grid-cols-2 gap-4'>
                {['Buyer', 'Seller'].map((userRole) => (
                  <motion.div 
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`relative cursor-pointer rounded-xl p-4 text-center transition-all ${role === userRole ? 'bg-indigo-50 border-2 border-indigo-500' : 'bg-gray-50 border-2 border-transparent hover:border-gray-200'}`}
                    onClick={() => setRole(userRole)}
                  >
                    <div className={`w-4 h-4 rounded-full border-2 absolute top-2 right-2 ${role === userRole ? 'bg-indigo-500 border-indigo-500' : 'border-gray-300'}`}>
                      {role === userRole && (
